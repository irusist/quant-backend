...
public class XueqiuApplication implements CommandLineRunner {

    private static final Logger log = LoggerFactory.getLogger(XueqiuApplication.class);

    public static void main(String[] args) {
        SpringApplication.run(XueqiuApplication.class, args);
    }
    ...
    public void run(String... strings) throws Exception {
        ...
        // 获取所有交易日
        List<String> bizDateList = jdbcTemplate.query("select biz_date from xueqiu_hs where code = '000001' order by biz_date",
        ...
        // 获取所有有基金的指数
        List<String> indexCodeList = jdbcTemplate.query("SELECT index_code from index_fund group by index_code order by index_code", new Object[0],
        ...
        for (String indexCode : indexCodeList) {
            ...
            // 查询 2017-09-29 日的成分股， choice数据只有截止到这天的数据
            List<IndexConstituent> constituentList = jdbcTemplate.query("select biz_date, stock_code, status from index_constituent_history a where index_code = ? and status = 1 and a.biz_date <= '2017-09-29' and (select count(*) from index_constituent_history where index_code = a.index_code and stock_code = a.stock_code and biz_date <= '2017-09-29' and biz_date > a.biz_date and status = 0) = 0",
                    new Object[]{indexCode}, (rs, rowNum) -> new IndexConstituent(rs.getString("biz_date"), rs.getString("stock_code"), rs.getInt("status")));
            log.info("constituent size is: " + constituentList.size());
            List<Valuation> valuationList = new ArrayList<>();
            // 获取成分记录第一天之后的数据
            List<IndexConstituent> constituentHistoryList = jdbcTemplate.query("SELECT * from index_constituent_history where index_code = ? and biz_date > ? order by biz_date", new Object[]{indexCode, "2017-09-29"},
    ...
    private Valuation getValuationOneDay(String indexCode, String bizDate, List<IndexConstituent> constituentList) {
        ...
        // 查询pe， pb
        String sql = "SELECT biz_date, code, pe_ttm, pb from xueqiu_hs where biz_date = ? and code in " + stockList.toString();
